...
void uwFPS_Init(void)
{
    ...
    if (gEngfuncs.pfnRegisterVariable) {
        ...
        // Служебная: сюда пишется оригинальный fps_max, пока он зажат.
        // FCVAR_ARCHIVE => переживает краш; проверяется в HUD_Frame.
        uw_saved_fps      = gEngfuncs.pfnRegisterVariable("uw_saved_fps",      "0",   FCVAR_ARCHIVE);
    }
...
void IPluginsV4::LoadClient(cl_exportfuncs_t* pExportFunc)
{
    ...
    g_pfnHUD_Reset    = pExportFunc->HUD_Reset;    pExportFunc->HUD_Reset    = HUD_Reset;
    g_pfnV_CalcRefdef = pExportFunc->V_CalcRefdef; pExportFunc->V_CalcRefdef = V_CalcRefdef;
}

// Восстановление здесь надёжно, т.к. RestoreFPS пишет через Cvar_SetValue,
// а не через командный буфер (который при выходе уже может не исполняться).
void IPluginsV4::ExitGame(int iResult) { RestoreFPS(); }

const char* IPluginsV4::GetVersion(void) { return UWFPS_VERSION; }
